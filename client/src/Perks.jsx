export default function Perks({selected, changePerkArray}) {
	function handleCbClick(ev) {
		const {checked, name} = ev.target;
		if (checked) {
			changePerkArray([...selected, name])
		} else {
			changePerkArray([...selected.filter(selectedName => selectedName !== name)]);
		}	
	}
	...
	return(
		<>
			...
			<label className="border p-4 flex rounded-2xl gap-2 items-center cursor-pointer">
				<input type="checkbox" checked={selected.includes("early check-in")} name="early check-in" onChange={handleCbClick}/>
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
